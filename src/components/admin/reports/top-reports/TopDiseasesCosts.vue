<template>
    <div>
        <v-layout justify-space-between align-center wrap mb-4>
            <h1 class="font-weight-medium grey--text text--darken-3">Top {{ number_selection === '5'? 'Five':'' || number_selection === '10'? 'Ten':'' || number_selection === '15'? 'Fifteen':'' }} Disease Base on Costs of Diagnostic and Therapeutic Procedures </h1>
        </v-layout>
        <v-row dense> 
            <v-col sm="6" md="6" xl="6">
                <div class="d-flex justify-start align-baseline">
                    ...
                    <div class="d-flex">
                        <v-btn  :disabled="loading_data" @click="topDieasesCosts" class="white--text ml-4" color="primary" height="41">
...
export default {
    ...
    mounted() {
        this.topDieasesCosts()
    },

    methods: {
        async topDieasesCosts() {
            ...
            let response = await this.$services.getAnnualCosting({
        ...
        exportToExcel() { 
            ...
            setTimeout(() => {
                ...
                this.$excel_file(this.export_to_excel_data, {
                    ...
                    fileName: `${file_name_facility} - Top ${this.select_top_number} Diseases based on costs ${this.selected_year} report - ${file_name_date}.xlsx`
                })
            }, 1000)
        } 
    }
