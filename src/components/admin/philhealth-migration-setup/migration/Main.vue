<template>
      <v-container class="pa-0">
        <section class="mx-auto">
          <h1 class="font-weight-medium grey--text text--darken-3 mb-4 mt-5">Philhealth Migration Setup</h1>
            <v-card width="900" class="py-5 mx-auto" flat>
                <v-card>
                    <v-card-title class="body-1 primary white--text">
                        <v-spacer></v-spacer>
                        <v-icon color="white" left>mdi-file-cloud</v-icon>
                        <span v-if="upload_type === 'ekonsulta'"> UPLOADING YAKAP XML FILE </span>
                        <span v-else-if="upload_type === 'eclaims'"> UPLOADING E-CLAIMS XML FILE </span>
                        <span v-else> SELECT UPLOAD TYPE </span>
                        <v-spacer></v-spacer>
                    </v-card-title>
                    ...
                    <v-row dense>
                        <v-col cols="12" sm="12" class="pa-5">
                          <v-row dense>
                            <v-col cols="12" class="pa-2 mb-10 mb-md-0">
                              <div class="d-flex justify-center align-center" style="height: 350px;" @drop.prevent="onDrop($event)" @dragover.prevent="upload_type ? drag_over = true : null" @dragenter.prevent="upload_type ? drag_over = true : null" @dragleave.prevent="upload_type ? drag_over = false : null" :class="{ 'grey lighten-2': drag_over }">
                                  <div class="pa-2">
                                          ...
                                          <v-tooltip bottom color="#1565C0">
                                              <template v-slot:activator="{ on }">
                                                  <div v-on="on" class="text-center bg-white">
                                                      ...
                                                      <label class="body-2 grey--text input-label">Browse your device</label>
                                      ...
                                      <template v-else>
                                          <div class="text-center">
                                              <h1 class="body-2 grey--text">Drag or drop files here</h1>
                                              <label for="file-upload" class="body-2 blue--text input-label" :class="{ 'grey--text lighten-2': !upload_type }">Browse your device</label>
                                              <v-file-input id="file-upload" accept=".xml" v-model="upload_files" @click="$event.target.value = ''" @change="uploadFile" :disabled="!upload_type" style="display:none"/>
                                          </div>
                                      </template>
                                  </div>
                              </div>
                              ...
                              <div class="file-items">
                                  ...
                                  <div v-if="file_migrated.length >= 1" class="my-4 ml-3">
                                      <div v-for="(files, key) in file_migrated" :key="key"> 
                                          ...
                                          <div v-if="upload_type === 'ekonsulta'">
                                          ...
                                          <div v-else>
                                                <v-btn @click="saveFailedMigration(files)" color="red" class="white--text" small> 
                                                    <v-icon class="mr-2" right dark> mdi-printer </v-icon>Save Eclaims Record Migration 
                                                </v-btn>
                                          </div>
                    ...
                    <div class="px-8"> 
                        <h4 class="caption red--text text--darken-2 font-italic"> " Note: To upload Yakap or eClaims XML file, choose first whether eClaims or Yakap to enable uploading file and use encrypted xml." </h4> 
                    </div>

                    <v-row class="px-8">
                        <v-col align-self="center" cols="12" sm="12">
                            <div class="d-flex">
                                <v-checkbox v-if="$store.state.prv_data.ekonsulta_phic_acc_no" label="Yakap" :input-value="upload_type === 'ekonsulta'" @change="upload_type = $event ? 'ekonsulta' : null" hide-details dense class="mr-4"/>
                                <v-checkbox v-if="$store.state.prv_data.eclaim_phic_acc_no" label="eClaims" :input-value="upload_type === 'eclaims'" @change="upload_type = $event ? 'eclaims' : null"  hide-details dense/>
                            </div>
                        </v-col>
                    </v-row>
                    ...
                    <v-card-actions class="d-flex justify-center">
                        <div class="mb-5">
                            <v-btn @click="submitFile" :disabled="file_name.length === 0 || is_success || !other_cipher_key && check_other_cipher_key || upload_loading || !upload_type" class="primary mx-2"> UPLOAD XML FILE </v-btn>
  ...
  methods: {
    ...
    submitFile() {
      ...
      this.all_files.forEach(async(file_data) => {
        // let response = await this.this.$services.uploadXmlMigration({
        //     provider_code: this.$store.state.prv_data.provider_code,
        //     file_data: file_data.files,
        //     file_name: file_data.file_name,
        //     cipher_key: this.check_other_cipher_key ? this.other_cipher_key : this.hci_cipher_key,
        //     phic_accre_no: this.upload_type === 'ekonsulta' ? this.$store.state.prv_data.ekonsulta_phic_acc_no : this.$store.state.prv_data.eclaim_phic_acc_no,
        //     migration_type: this.upload_type
        // })
        const payload = {
          provider_code: this.$store.state.prv_data.provider_code,
          file_data: file_data.files,
        ...
        let response = await (this.upload_type === 'eclaims' ? this.$services.eclaimUploadMigration(payload) : this.$services.uploadXmlMigration(payload))
        ...
        if (response.status === 200) {
          ...
          if(response.data.success) {
            ...
            this.alert = { display: true, type: 'standard', width: '350', icon: 'mdi-alert-circle', color: 'red', title: this.upload_type === 'ekonsulta' ? 'Ekonsulta Migration' : 'Eclaims Migration', body: 'Migration failed', btn_pry_txt: 'OK', btn_pry_color: 'primary', btn_pry_otl: false, btn_pry_act: 'closeAlert' }
          }

          if(response.data.message === 'incorrect cipher key') {
            this.error_message = 'Invalid Cipher Key'
          } else if(response.data.message === 'invalid encrypted xml file') {
    ...
    saveFailedMigration(files){
      this.migration_items = files

      var doc = new this.jspdf('p', 'mm', 'letter')
      const center = doc.internal.pageSize.getWidth() / 2
      doc.setFontSize(14)
      doc.setFont('helvetica', 'bold')
      doc.text(this.upload_type === 'ekonsulta' ? 'Ekonsulta Failed Migration' : 'Eclaims Migration Record', center, 12, 'center')
      let base_height = 0

      if(this.upload_type === 'ekonsulta') {
        doc.setFontSize(10)
        ...
        this.migration_items.failed_records_list.forEach(data => {
          doc.setFontSize(8)
        ...
        doc.setFontSize(10)
        doc.text('Claim No', 8, base_height + 20)
        doc.text('First Name', 50, base_height + 20)
        doc.text('Middle Name', 80, base_height + 20)
        doc.text('Last Name', 112, base_height + 20)
        doc.text('Date of Birth', 136, base_height + 20)
        doc.text('PHIC Pin', 166, base_height + 20)
        doc.text('Status', 192, base_height + 20)

        this.migration_items.record_list.forEach(data => {
          doc.setFontSize(8)
          doc.setFont('helvetica', 'regular')
          doc.text(data.pClaimNo, 8, base_height + 30)
          doc.text(data.first_name, 60, base_height + 30, 'center')
          doc.text(data.middle_name, 90, base_height + 30, 'center')
          doc.text(data.last_name, 122, base_height + 30, 'center')
          doc.text(data.birthdate, 146, base_height + 30, 'center')
          doc.text(data.phic_pin, 166, base_height + 30)
          doc.text(data.status.toUpperCase(), 192, base_height + 30)
      ...
      doc.save('Migration Records' + this.migration_items.file_name +  ' .pdf')
    },
