<template>
  <v-container fluid px-6>
    ...
    />
     ...
     <!-- TABLE FOR ADMISSION -->
      <v-data-table v-model="selected_queue" :headers="doctor_header_list" :items="filtered_items" :loading="loading_tbl" :mobile-breakpoint="0" class="elevation-2 mb-5" :show-select="show_cancel_checkbox" item-key="transaction_number">
        <template v-slot:header.data-table-select="{ props, on }">
          ...
          <template v-if="!show_cancel_checkbox" v-slot:item="{ item }">
            <tr v-on:click="selectRow(item)" @click="transaction_number = item.transaction_number" :class="{'grey lighten-2' : item.transaction_number === transaction_number}">
                ...
                <td>{{ [item.last_name, item.first_name].includes(null) ? 'N/A' : (item.middle_name ? item.first_name + ' ' + item.middle_name + ' ' + item.last_name : item.first_name + ' ' + item.last_name) }}</td>
                <td>{{ item.transaction_number }}</td>
                <td>{{ item.mem_priority_care }}</td>
...
export default {
  ...
  data(){
    return{
      ...
      header_title: "Patient's Queue List",
      ...
      loading_tbl: true,
      doctor_header_list:[
        { text: 'Date Admitted', value: 'date_created',sortable: false, width:"200px" },
        ...
        { text: 'Priority Care',sortable: false, value: 'mem_priority_care', width: '200px'},
        ...
        { text: 'Endorsed By', value: 'endorsed_by', sortable: false,width:"250px"  },
  ...
  computed:{
    ...
    filtered_items() {
      return this.doctor_items.filter((i) => {
        ...
        let searched_data = member_name.toLowerCase().indexOf(this.search_tbl.toLowerCase()) > -1 ||
          (i.transaction_number || '').toLowerCase().indexOf(this.search_tbl.toLowerCase()) > -1 ||
          (i.processing_by || '').toLowerCase().indexOf(this.search_tbl.toLowerCase()) > -1 ||
          (i.type_of_visit || '').toLowerCase().indexOf(this.search_tbl.toLowerCase()) > -1;
        ...
        const priority_order = {'SENIOR CITIZEN': 3,'PWD': 3,'PREGNANT WOMAN': 3,'REGULAR PATIENT': 0};
        const normalizeStatus = (status) => {
          if (status === 'TO PROCESS') return 'ON QUEUE';
          if (status.includes('ON PROCESS')) return 'ON PROCESS';
          return status;
        };
        const status_order = {'ON PROCESS': 2,'ON QUEUE': 1};

        const a_date_only = a.date_created.substring(0, 10);
        const b_date_only = b.date_created.substring(0, 10);

        if (a_date_only !== b_date_only) {
          return new Date(a_date_only) - new Date(b_date_only);
        }

        //Priority within same date
        const a_priority = priority_order[a.mem_priority_care] || 0;
        const b_priority = priority_order[b.mem_priority_care] || 0;
        if (b_priority !== a_priority) {
          return b_priority - a_priority;
        }

        //Transaction Status
        const a_status = status_order[normalizeStatus(a.transaction_status)] || 0;
        const b_status = status_order[normalizeStatus(b.transaction_status)] || 0;
      ...
      if (this.transaction_number == item.transaction_number) {
        ...
        if (item.transaction_status === 'ON PROCESS DOCTOR' || item.transaction_status === 'ON PROCESS MIDWIFE') {
            session_data = {
              ...
              from_component: 'p_queue_list',
              editable: true,
              routes: 'EMRConsultation'
            }
            sessionStorage.setItem('PIX235', this.$crypto.AES.encrypt(JSON.stringify(session_data),'EMX246'))
            this.$store.state.usr_credentials.user_settings.user_layout === 'type_a' ? this.$router.replace('/emr-consultation/patient-information') : this.$router.replace('/emr-consultation')
        } else {

          let response = await this.$services.trnUpdateTranRecordStatus({
              transaction_number: item.transaction_number,
              transaction_status: `ON PROCESS ${this.$store.state.usr_credentials.department.toUpperCase()}`
          })
