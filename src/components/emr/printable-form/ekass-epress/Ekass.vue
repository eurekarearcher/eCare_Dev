<template>
    <v-dialog v-model="show_ekass" width="816px" persistent>
        <div id="for-print-absolute">
            <v-card  id="for-print-relative" class="px-4 pt-4 pb-1" flat>
                ...
                <div v-else id="for-print-ekass">
                    <v-row dense class="mb-2">
                        <v-col sm="12" md="12" lg="12">
                            <div class="border-first-ekas">
                                <div class="border-second pa-2">
                                    <v-row dense>
                                        ...
                                        <v-col sm="10" md="10" lg="10" class="d-flex justify-start align-center ma-0 pt-0">
                                            <span class="ma-0 text-h6 font-weight-bold"> ELECTRONIC KONSULTA AVAILMENT SLIP (EKAS)</span>  
                                        </v-col>
                                    ...
                                    <v-simple-table cellspacing="0" class="mt-1"> 
                                        <tr >  
                                            <td class="text-start line-height-2" width="5%"> 
                                                ...
                                                <span class="font-weight-regular text-small text-small"> {{ this.$store.state.prv_data.provider_name }} </span>
                                            </td>
                                            <td class="text-start line-height-2" width="1%"> 
                                                <span class="font-weight-medium text-small"> Case No: </span> <br/>
                                                <span class="font-weight-regular text-small text-small"> {{ ekonsulta_data.pHciCaseNo }} </span>
                                            </td>
                                            <td class="text-start line-height-2" width="5%"> 
                                                <span class="font-weight-medium text-small"> HCI Accreditation No.: </span> <br/>
                                                <span class="font-weight-regular text-small text-small"> {{ ekonsulta_data.pHciAccreNo }} </span>
                                            </td>
                                            <td class="text-start line-height-2" width="5%"> 
                                                <span class="font-weight-medium text-small"> Transaction No.: </span> <br/>
                                                <span class="font-weight-regular text-small text-small"> {{ transaction_number }} </span>
                                            </td>
                                        </tr>
                                    </v-simple-table>
                                    ...
                                    <v-simple-table cellspacing="0" >
                                        ...
                                        <!-- <tbody>
                                            <tr class="table-body-ekass">
                                                <td style="text-align: left !important;">Consultation</td>
                                                <td >{{ history_and_pe.status  === 'Done'? '&#10004;':'X' }} </td>
                                                <td>{{ history_and_pe.trans_date }} </td>
                                                <td>{{ prescribed_doctor }}</td>
                                            </tr>
                                
                                            <tr class="table-body-ekass">
                                                <td style="text-align: left !important">History and physical examination (vitals, anthropometrics)</td>
                                                <td>{{ history_and_pe.status  === 'Done'? '&#10004;':'X' }} </td>
                                                <td>{{ history_and_pe.trans_date }} </td>
                                                <td></td>
                                            </tr>
                                    ...
                                    <v-row dense no-gutters class="mt-2">
                                        ...
                                        <v-col sm="12" md="12" lg="12" class="d-flex justify-start align-center ma-0 pa-0">
                                            <v-col sm="7" md="7" lg="7" class="ma-0 pa-0">
                                                <span class="ma-0 pa-0 line-height"> Have you received the above-mentioned essential services? <br>(Natanggap mo ba ang mga essential services na nabanggit?) </span>  
                                            </v-col>
                                        ...
                                        <v-col sm="12" md="12" lg="12" class="d-flex justify-start align-center ma-0 pa-0">
                                            <v-col sm="7" md="7" lg="7" class="ma-0 pa-0"> 
                                                ...
                                                (Gaano ka nasiyahan sa natanggap mong serbisyo?) </span>  
                                            </v-col>
                                            <v-col sm="5" md="5" lg="5" class="d-flex justify-start ma-0 pa-0">
...
export default {
    props: ['selected_icd', 'prescribed_doctor', 'transaction_number', 'patient_info', 'ekonsulta_data', 'diagnostic_procedure', 'atc_code', 'history_and_pe', 'loading_data', 'show_ekass'],
    ...
    methods: {
        ...
        printEkassEpress() {
            ...
            if (this.patient_info) {
                html2canvas(document.getElementById('for-print-ekass')).then(function(canvas) {
                    ...
                    doc.save(`${mem_first_name} ${mem_middle_name} ${mem_last_name} ${mem_suffix} EKAS.pdf`);
                });
            }
        }
    }
