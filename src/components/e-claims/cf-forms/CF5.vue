  ...
  methods:{
    async getPDF(){
        ...
        if(response.status === 200){
            ...
            if(response.cf5){
                this.pdf_url = await this.$get_pdf_forms.getCF5(response.cf5, this.cforms_data)
            }
        }
    },
    ...
    downloadPDF(){
      ...
      link.download =   `${patient_name}_${file_name_facility}_CF5.pdf`;
