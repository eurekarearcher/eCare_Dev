...
<script>
export default {
  props: ['cf1_data_p1', 'cf1_data_p2', 'cforms_data'],
  data(){
    return{
  ...
  methods:{
    async getPDF(){
        ...
        if(response.status === 200){
            ...
            if(response.cf4){
                this.pdf_url = await this.$get_pdf_forms.getCF4(response.cf4, this.cforms_data)
            }
        }
    },
    ...
    downloadPDF(){
      ...
      link.download =   `${patient_name}_${file_name_facility}_CF4.pdf`;
